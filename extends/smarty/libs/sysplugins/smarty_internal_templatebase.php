...
abstract class Smarty_Internal_TemplateBase extends Smarty_Internal_Data
{
    ...
    {
        // display template
        $path = array_values(array_filter(explode("/",$template)));
        if(count($path) == 1){
            $template = $path[0]."/view/".APP_CONTROLLER."/".APP_ACTION.TPL;
        }elseif(count($path) == 2){
            $template = $path[0]."/view/".$path[1]."/".APP_ACTION.TPL;
        }elseif(count($path) == 3){
            $template = $path[0]."/view/".$path[1]."/".$path[2].TPL;
        }
